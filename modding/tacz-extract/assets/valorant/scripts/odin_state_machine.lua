...
local BARREL_TRACK =increment(static_track_top)
...
function base_track_state.entry(this, context)
    ...
    context:runAnimation("barrel", context:getTrack(STATIC_TRACK_LINE, BARREL_TRACK), false, PLAY_ONCE_HOLD, 0)
    context:setAnimationProgress(context:getTrack(STATIC_TRACK_LINE, BARREL_TRACK),1,true)
end
...
function bolt_caught_states.normal.entry(this, context)
    ...
    context:runAnimation("ammo_display", context:getTrack(STATIC_TRACK_LINE, BOLT_CAUGHT_TRACK), true, PLAY_ONCE_HOLD, 0)
...
function bolt_caught_states.normal.update(this, context)
    ...
    if (isNoAmmo(context)) then
        ...
        local a = math.min(context:getAmmoCount(),8)
        context:setAnimationProgress(context:getTrack(STATIC_TRACK_LINE, BOLT_CAUGHT_TRACK), 0.1*(8-a), false)
    end
end
...
function gun_kick_state.transition(this, context, input)
    ...
    if (input == INPUT_SHOOT) then
        ...
        if (current_timestamp - last_shoot_timestamp) >= (2*shoot_interval + 50) then
            base_track_state.accel = 0
        else
            local accel = base_track_state.accel
            base_track_state.accel = math.min(accel + 1,accel_count)
        end
        local final_adjust = 0
        local rate = (param.rpm_rate)-1
        local accel_count = 20
        local shoot_interval = context:getShootInterval()
        if (context:getAimingProgress()>0.5) then
            rate = rate
        else
            rate = rate*(base_track_state.accel/accel_count)
        end
        if not (rate<=0) then
            final_adjust = (1/(rate+1))-1
        end
        context:adjustClientShootInterval(shoot_interval*final_adjust)
    end
    return nil
end
