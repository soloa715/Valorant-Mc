...
for base_item in BASE_ITEMS:
    item_json = {
        "parent": f"minecraft:item/handheld" if "sword" in base_item or "axe" in base_item or "pickaxe" in base_item or "hoe" in base_item else f"minecraft:item/{base_item}",
        "textures": {
            "layer0": f"minecraft:item/{base_item}"
        },
        "overrides": sorted(overrides, key=lambda x: x["predicate"]["custom_model_data"])
    }
    item_path = os.path.join(RP_BASE, "assets", "minecraft", "models", "item", f"{base_item}.json")
    with open(item_path, "w") as f:
        json.dump(item_json, f, indent=2)
print("  All base item JSON overrides updated for all weapons + skin variants")
