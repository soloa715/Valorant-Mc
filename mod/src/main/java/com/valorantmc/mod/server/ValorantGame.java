...
public class ValorantGame {
    ...
    public void start(MinecraftServer server) {
        ...
        if (attackerSpawns.isEmpty()) {
            ...
            if ("Arena".equalsIgnoreCase(chosenMap) || attackerSpawns.stream().anyMatch(v -> v.y >= 100)) {
                // Spawns use arena coords — build the floating arena platform at Y=150
    ...
    private void tickRoundActive(MinecraftServer server) {
        if (spikePlanted && spikeDetonTicks > 0) {
            ...
            // Spike ticking audio & visual particle ring
            if (spikePlantPos != null && server.overworld() != null) {
                ServerLevel level = server.overworld();
                int interval = spikeDetonTicks > 300 ? 20 : (spikeDetonTicks > 150 ? 10 : 5);
                if (spikeDetonTicks % interval == 0) {
                    float pitch = 1.0f + (1.0f - ((float) spikeDetonTicks / SPIKE_TICKS));
                    level.playSound(null, spikePlantPos.x, spikePlantPos.y, spikePlantPos.z,
                            net.minecraft.sounds.SoundEvents.NOTE_BLOCK_PLING,
                            net.minecraft.sounds.SoundSource.BLOCKS, 1.2f, pitch);
                    
                    for (int degree = 0; degree < 360; degree += 45) {
                        double rad = Math.toRadians(degree);
                        double rx = spikePlantPos.x + Math.cos(rad) * 1.5;
                        double rz = spikePlantPos.z + Math.sin(rad) * 1.5;
                        level.sendParticles(net.minecraft.core.particles.ParticleTypes.FLAME, rx, spikePlantPos.y + 0.5, rz, 1, 0, 0, 0, 0.01);
                    }
                }
            }

            if (spikeDetonTicks <= 0) {
                // Clear 3D Spike block
                if (spikePlantPos != null && server.overworld() != null) {
                    BlockPos bp = BlockPos.containing(spikePlantPos);
                    server.overworld().setBlock(bp.above(), Blocks.AIR.defaultBlockState(), 3);
                    server.overworld().setBlock(bp, Blocks.AIR.defaultBlockState(), 3);
                }
    ...
    public void plantSpike(ServerPlayer player, MinecraftServer server) {
        ...
        // Place physical 3D Spike block
        ServerLevel level = player.serverLevel();
        BlockPos bp = BlockPos.containing(spikePlantPos);
        level.setBlock(bp, Blocks.BLACKSTONE.defaultBlockState(), 3);
        level.setBlock(bp.above(), Blocks.REDSTONE_TORCH.defaultBlockState(), 3);
